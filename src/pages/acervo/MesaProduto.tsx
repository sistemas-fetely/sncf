...
export default function MesaProduto() {
  ...
  const carregando=lista.isLoading||conc.isLoading||sugestoes.isLoading||fasesDim.isLoading||impactosDim.isLoading||regrasDim.isLoading||anunciosSemProduto.isLoading||separadosDim.isLoading; const erro=lista.error??conc.error??sugestoes.error??fasesDim.error??impactosDim.error??regrasDim.error??anunciosSemProduto.error??separadosDim.error;
  ...
  function aplica(l:LinhaUnida, ignorar?:GrupoFiltro, semIndicador=false){
    ...
    if(!semIndicador&&indicador){if(indicador.startsWith("sep:")){if(l.fase!=="ativo"||!(l.falta_separada??[]).includes(indicador.slice(4)))return false;}else{const slug=indicador==="prontos"?"pronto_para_ativo":indicador==="bloqueados"?"bloqueado":"ativo_com_furo";if(l.sugestao!==slug)return false;}}
    return true;
  }
  ...
  const anuncios=(anunciosSemProduto.data??[]) as {gravidade:number|null}[];
  const cards=[{id:null as Indicador,label:"Total",n:linhas.filter(l=>aplica(l,undefined,true)).length,tooltip:null as string|null,gravidade:null as number|null},{id:"prontos" as Indicador,label:"Prontos para promover",n:conta(l=>l.sugestao==="pronto_para_ativo"),tooltip:null,gravidade:null},{id:"bloqueados" as Indicador,label:"Bloqueados",n:conta(l=>l.sugestao==="bloqueado"),tooltip:null,gravidade:null},{id:"furo" as Indicador,label:"Furo em ativo",n:conta(l=>l.sugestao==="ativo_com_furo"),tooltip:null,gravidade:null},...(separadosDim.data??[]).map(s=>({id:`sep:${s.campo}` as Indicador,label:s.rotulo_separado??s.campo,n:conta(l=>l.fase==="ativo"&&(l.falta_separada??[]).includes(s.campo)),tooltip:"Exigido para ser ativo, mas mostrado à parte do Furo em ativo.",gravidade:null})),...impactosOrdenados.map(i=>({id:`imp:${i.slug}` as Indicador,label:i.nome,n:conta(l=>l.fase==="ativo"&&(l.impactos??[]).includes(i.slug)),tooltip:i.descricao,gravidade:i.gravidade})),...(anunciosSemProduto.data?[{id:"anuncios" as Indicador,label:"Anúncios sem produto",n:anuncios.length,tooltip:"Anúncios ativos no Shopify que não ligam a nenhum produto do cadastro (ex.: variante sem SKU). Não entram nos cards de produto.",gravidade:anuncios.reduce((m,a)=>Math.max(m,a.gravidade??0),0)}]:[])];
  ...
  <section className="grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-6" aria-label="Indicadores">{carregando?Array.from({length:6}).map((_,i)=><Skeleton key={i} className="h-20"/>):cards.map(c=>{const impacto=String(c.id??"").startsWith("imp:");const anuncio=c.id==="anuncios";const selecionado=indicador===c.id;const botao=<Button key={c.label} variant="outline" className={cn("h-20 items-start justify-center border p-3 text-left",(impacto||anuncio)&&tomCardImpacto(c.gravidade,c.n),selecionado&&"ring-2 ring-primary ring-offset-2 ring-offset-background")} onClick={()=>{if(anuncio){navigate("/vendas/produto/conciliacao?sem_produto=1&fase=ativo");return;}if(impacto){navigate(`/vendas/produto/conciliacao?impacto=${encodeURIComponent(String(c.id).slice(4))}&fase=ativo`);return;}setIndicador(selecionado?null:c.id);}}><span className="flex w-full flex-col"><span className="text-[11px] font-normal text-muted-foreground">{c.label}</span><span className={cn("mt-1 text-[21px] font-medium tabular-nums",(impacto||anuncio)?tomNumeroImpacto(c.gravidade,c.n):"text-foreground")}>{c.n}</span></span></Button>;return c.tooltip?<Tooltip key={c.label}><TooltipTrigger asChild>{botao}</TooltipTrigger><TooltipContent className="max-w-xs">{c.tooltip}</TooltipContent></Tooltip>:botao;})}</section>
