...
function PainelSku({
  ...
  const open = sku != null;
  const cockpitRow = useMemo(
    () => (sku ? rowCache.find((r) => r.sku === sku) ?? null : null),
    [sku, rowCache],
  );

  const detalheQuery = useQuery({
    queryKey: ["sncf_produtos", sku],
    enabled: open,
    queryFn: async (): Promise<SncfProdutoDetalhe | null> => {
      ...
      const { data, error } = await (supabase as any)
        ...
        .maybeSingle();
      if (error) throw error;
      return (data ?? null) as SncfProdutoDetalhe | null;
    },
  });

  const movQuery = useQuery({
    queryKey: ["movimentacao_estoque", sku],
    enabled: open,
  ...
  const reservandoQuery = useQuery({
    queryKey: ["pedido_itens_reservando", sku],
    enabled: open,
    queryFn: async (): Promise<PedidoReservandoRow[]> => {
      // Estágios que efetivamente reservam estoque
      const estagiosReserva = [
        "aguardando_estoque", "em_separacao", "em_conferencia",
        "aguardando_faturamento", "faturado", "aguardando_coleta",
        "em_transporte",
      ];
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const { data, error } = await (supabase as any)
        .from("pedido_itens")
        .select("quantidade, pedido:pedidos!inner(id, id_externo, cliente_nome, estagio)")
        .eq("sku", sku!)
        .in("pedido.estagio", estagiosReserva)
        .limit(20);
      if (error) throw error;
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return ((data ?? []) as any[]).map((r) => ({
        ...
        cliente_nome: r.pedido?.cliente_nome,
        quantidade: r.quantidade,
        estagio: r.pedido?.estagio,
      })).filter((r) => r.pedido_id);
    },
  });
...
SKU: ${c?.sku ?? sku ?? ""}
Produto: ${c?.nome_comercial ?? ""}
Campo a corrigir: 
Valor atual: 
Valor correto: 
Solicitado por: SNCF · Cockpit de Produto · ${new Date().toLocaleDateString("pt-BR")}`;
    navigator.clipboard.writeText(texto).then(
      () => toast.success("Solicitação copiada", { description: "Cole no FOP para corrigir." }),
        ...
        {sku && (
          <>
            <SheetHeader>
              ...
              <SheetDescription>
                {cockpitRow?.nome_comercial ?? det?.nome_comercial ?? ""}
              </SheetDescription>
            </SheetHeader>
            ...
            <div className="mt-6 space-y-6">
              ...
              <Secao titulo="Cadastro (FOP)">
                <div className="rounded-md border bg-muted/30 p-3 mb-3 text-xs flex items-start gap-2">
                  <Info className="h-4 w-4 mt-0.5 flex-shrink-0 text-muted-foreground" />
                  <div className="flex-1">
                    <p className="text-muted-foreground">
                      Cadastro e preço são do FOP. Correções devem ser feitas lá e refletem aqui automaticamente.
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      <Copy className="h-3 w-3" /> Copiar solicitação de correção
                    </Button>
                  </div>
                </div>
                <Grid2>
                  <Field label="SKU" value={det?.sku ?? sku} mono />
                  <Field label="EAN" value={det?.ean} mono />
                  <Field label="Nome completo" value={det?.nome_completo ?? det?.nome_comercial} colSpan={2} />
                  <Field label="Marca" value={det?.marca} />
                  <Field label="Linha" value={det?.linha} />
                  ...
                  <Field
                    label="Dimensões (A×L×P cm)"
                    value={
                      det?.altura_cm || det?.largura_cm || det?.profundidade_cm
                        ? `${formatNum(det?.altura_cm, 1)} × ${formatNum(det?.largura_cm, 1)} × ${formatNum(det?.profundidade_cm, 1)}`
                        : null
                    }
                    colSpan={2}
                  />
              ...
              <Secao titulo="Comercial">
                <Grid2>
                  <Field label="Preço B2B" value={cockpitRow?.preco_b2b == null ? null : formatBRL(cockpitRow.preco_b2b)} />
                  <Field label="Preço B2C" value={cockpitRow?.preco_b2c == null ? null : formatBRL(cockpitRow.preco_b2c)} />
                  <Field
                    label="Custo"
                    value={
                      cockpitRow?.custo == null ? null : (
                        <span className="flex items-center gap-1.5">
                          {formatBRL(cockpitRow.custo)}
                          {cockpitRow.custo_status === "interino" && (
                            <Badge variant="outline" className="text-[10px] px-1 py-0 bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20">interino</Badge>
                          )}
                          {cockpitRow.custo_status === "real" && (
                            <Badge variant="outline" className="text-[10px] px-1 py-0 bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20">real</Badge>
                          )}
                        </span>
                      )
                    }
                  ...
                  <Field
                    label="Margem B2B"
                    value={
                      <span className={cockpitRow?.abaixo_piso_b2b ? "text-red-600 dark:text-red-400 font-medium" : ""}>
                        {formatPct(cockpitRow?.resultado_pct_b2b)}
                        {cockpitRow?.abaixo_piso_b2b && <Badge variant="outline" className="ml-2 text-[10px] px-1 py-0 bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20">abaixo do piso</Badge>}
                      </span>
                    }
                  />
                  <Field
                    label="Margem B2C"
                    value={
                      <span className={cockpitRow?.abaixo_piso_b2c ? "text-red-600 dark:text-red-400 font-medium" : ""}>
                        {formatPct(cockpitRow?.resultado_pct_b2c)}
                        {cockpitRow?.abaixo_piso_b2c && <Badge variant="outline" className="ml-2 text-[10px] px-1 py-0 bg-red-500/10 text-red-700 dark:text-red-400 border-red-500/20">abaixo do piso</Badge>}
                      </span>
                    }
                  />
                </Grid2>
                {cockpitRow?.preco_divergente_bling && (
                  <div className="mt-3 rounded-md border border-amber-500/40 bg-amber-500/5 p-3 text-xs">
                    <div className="flex items-start gap-2">
                      <AlertTriangle className="h-4 w-4 text-amber-600 dark:text-amber-400 mt-0.5 flex-shrink-0" />
                      <div>
                        <div className="font-medium text-amber-800 dark:text-amber-300">Preço divergente do Bling</div>
                        <div className="mt-1 text-muted-foreground">
                          FOP: <span className="font-medium text-foreground">{formatBRL(cockpitRow.preco_b2c)}</span>
                          {" · "}
                          Bling: <span className="font-medium text-foreground">{formatBRL(cockpitRow.preco_no_bling)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              <Secao titulo="Venda">
                <Grid2>
                  <Field label="Unidades vendidas" value={formatNum(cockpitRow?.un_vendidas)} />
                  <Field label="Receita" value={formatBRL(cockpitRow?.receita ?? 0)} />
                  <Field label="Pedidos" value={formatNum(cockpitRow?.pedidos)} />
                  <Field label="Clientes" value={formatNum(cockpitRow?.clientes)} />
                  <Field label="Última venda" value={formatDateBR(cockpitRow?.ultima_venda)} />
                  <Field label="Dias sem vender" value={cockpitRow?.dias_sem_vender == null ? null : `${formatNum(cockpitRow.dias_sem_vender)}d`} />
                  <Field label="Un/dia" value={formatNum(cockpitRow?.un_por_dia, 2)} />
                  <Field label="Un canceladas" value={formatNum(cockpitRow?.un_canceladas)} />
                  <Field label="Receita cancelada" value={formatBRL(cockpitRow?.receita_cancelada ?? 0)} colSpan={2} />
                </Grid2>
              </Secao>
              ...
              <Secao titulo="Estoque">
                <Grid2>
                  <Field label="Base" value={formatNum(cockpitRow?.estoque_base)} />
                  <Field label="Reservado" value={formatNum(cockpitRow?.reservado)} />
                  <Field
                    label="Virtual"
                    value={
                      <span className={Number(cockpitRow?.estoque_virtual ?? 0) < 0 ? "text-red-600 dark:text-red-400 font-medium" : ""}>
                        {formatNum(cockpitRow?.estoque_virtual)}
                      </span>
                    }
                  />
                  <Field
                    label="Fonte"
                    value={
                      cockpitRow?.tem_razao ? (
                        <Badge variant="outline" className="font-normal bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-500/20">Lastreado</Badge>
                      ) : (
                        <Badge variant="outline" className="font-normal bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-500/20">Saldo Bling</Badge>
                      )
                    }
                  />
                  <Field label="Status" value={cockpitRow?.status_venda ?? "—"} />
                  <Field label="Dias desde contagem" value={cockpitRow?.dias_desde_contagem == null ? null : `${formatNum(cockpitRow.dias_desde_contagem)}d`} />
                  <Field label="Cobertura" value={cockpitRow?.cobertura_dias == null ? null : `${formatNum(cockpitRow.cobertura_dias)}d`} />
                  <Field label="Capital parado" value={cockpitRow?.capital_parado == null ? null : formatBRL(cockpitRow.capital_parado)} />
                </Grid2>
              </Secao>

              {/* Movimentação */}
              <Secao titulo="Movimentação (últimas 20)">
                {movQuery.isLoading ? (
                  <div className="text-xs text-muted-foreground py-4 text-center">Carregando…</div>
                ) : (movQuery.data ?? []).length === 0 ? (
                  <div className="text-xs text-muted-foreground py-4 text-center">
                    Sem movimentação no razão — SKU ainda não onboardado.
                  </div>
                ) : (
                  <div className="rounded-md border overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {(movQuery.data ?? []).map((m, i) => (
                          <TableRow key={i}>
                            <TableCell className="text-xs">{formatDateBR(m.data_mov)}</TableCell>
                            <TableCell className="text-xs">{m.tipo ?? "—"}</TableCell>
                            <TableCell className="text-xs text-right tabular-nums">{formatNum(m.quantidade)}</TableCell>
                            <TableCell className="text-xs text-muted-foreground">
                              {m.referencia ?? m.origem ?? "—"}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
              ...
              <Secao titulo="Pedidos reservando">
                {reservandoQuery.isLoading ? (
                  <div className="text-xs text-muted-foreground py-4 text-center">Carregando…</div>
                ) : (reservandoQuery.data ?? []).length === 0 ? (
                  <div className="text-xs text-muted-foreground py-4 text-center">
                    Nenhum pedido reservando este SKU.
                  </div>
                ) : (
                  <div className="rounded-md border overflow-hidden">
                    <Table>
                      ...
                      <TableBody>
                        {(reservandoQuery.data ?? []).map((r) => (
                          <TableRow key={r.pedido_id}>
                            <TableCell className="text-xs font-mono">{r.id_externo ?? r.pedido_id.slice(0, 8)}</TableCell>
                            <TableCell className="text-xs">{r.cliente_nome ?? "—"}</TableCell>
                            <TableCell className="text-xs text-right tabular-nums">{formatNum(r.quantidade)}</TableCell>
                            <TableCell className="text-xs text-muted-foreground">{r.estagio ?? "—"}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
...
function Secao({ titulo, children }: { titulo: string; children: React.ReactNode }) {
  return (
    <section>
      <h3 className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">{titulo}</h3>
      {children}
    </section>
  );
}
...
function Field({
  ...
  return (
    <div className={cn(colSpan === 2 && "col-span-2")}>
      <div className="text-[10px] uppercase tracking-wider text-muted-foreground">{label}</div>
      <div className={cn("text-sm mt-0.5", mono && "font-mono")}>
