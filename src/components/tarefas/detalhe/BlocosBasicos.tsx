...
export function BlocoCampos({ tarefa }: { tarefa: TarefaDetalhe }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3">
      ...
      {/* Vínculo com a entidade de origem: os três campos andam juntos, sempre.
          `tipo_origem` NÃO muda — quem criou e do que trata são coisas diferentes. */}
      <Campo rotulo="Vinculado a">
        <SeletorVinculoTarefa
          moduloOrigem={tarefa.modulo_origem}
          entidadeId={tarefa.entidade_origem_id}
          acaoUrl={tarefa.acao_url}
          onChange={(v) => salvar.mutate(v)}
        />
      </Campo>
    </div>
