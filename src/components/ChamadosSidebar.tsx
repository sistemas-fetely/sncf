/**
 * ChamadosSidebar — MENU-VIA-TABELA + CHAMADOS-É-APP-PRÓPRIO (15/09/2026).
 *
 * Cópia estrutural da MeuEspacoSidebar: itens, rótulos, ícones e ordem vêm da
 * sncf_navegacao (app "chamados") — grupos Fila (8 visões da fila) e Operação
 * (Mapa da Operação, Catálogo de Serviços). Antes as rotas de chamados
 * moravam no VendasLayout e a Central abria com a sidebar do SOPs.
 */
...
export function ChamadosSidebar() {
  const { state } = useSidebar();
  const collapsed = state === "collapsed";
  const location = useLocation();
  const { grupos, isLoading } = useMenuApp("chamados");
        ...
        {collapsed ? (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex min-w-0 flex-col">
              <span className="text-sm font-medium tracking-tight text-sidebar-foreground">Chamados</span>
              <span className="text-[11px] text-sidebar-muted">A fila do suporte interno</span>
            </div>
