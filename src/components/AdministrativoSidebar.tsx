 ...
 * (temPermissaoTela / TELAS_PUBLICAS / useAuth().roles): o cardápio não pode
 * oferecer porta trancada. O gate segue sendo a última linha de defesa.
 */
...
function useVisibilidadeMenuFixo() {
  ...
  const podeVer = (rota: string) => {
    ...
    // Item de menu fora da sncf_navegacao: mostra (some em silêncio é pior).
    if (!r) return true;
