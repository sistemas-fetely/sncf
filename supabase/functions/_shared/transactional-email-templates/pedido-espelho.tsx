...
const PedidoEspelhoEmail = ({
  ...
  condicao_pagamento,
  valor_liquido,
  tem_planilha_cadastro,
}: PedidoEspelhoProps) => {
  return (
    <Html lang="pt-BR" dir="ltr">
      <Head />
      <Preview>Fetély · Segue o seu pedido em anexo.</Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={body}>
            <Text style={headlineNormal}>
              <span style={headlineBold}>Segue o seu pedido.</span>
            </Text>

            <Text style={sublineStyle}>
              {parceiro_nome ? `Olá, ${parceiro_nome}. ` : 'Olá. '}
              {pedido_id_externo
                ? `O pedido ${pedido_id_externo} está anexado a este e-mail.`
                : 'O seu pedido está anexado a este e-mail.'}
            ...
            <Section style={cardSection}>
              ...
              {forma_pagamento && (
                <Row style={infoRow}>
                  ...
                  <Column style={infoValue}>{forma_pagamento}</Column>
                </Row>
              )}
              {condicao_pagamento && (
                <Row style={infoRow}>
                  <Column style={infoLabel}>Condição</Column>
                  <Column style={infoValue}>{condicao_pagamento}</Column>
                </Row>
              )}
              {valor_liquido && (
                <Row style={infoRow}>
            ...
            <Text style={ctaNote}>
              {tem_planilha_cadastro
                ? 'Em anexo: o pedido em PDF e a planilha de cadastro dos produtos.'
                : 'O pedido em PDF está em anexo neste e-mail.'}
            </Text>
          </Section>
...
export const template: TemplateEntry = {
  component: PedidoEspelhoEmail,
  subject: (data) =>
    `[Fetely] Seu pedido ${data.pedido_id_externo ?? ''}`.trim(),
  displayName: 'Espelho do Pedido (PDF em anexo)',
  previewData: {
    parceiro_nome:         'Bella Decorações',
    pedido_id_externo:     'PED-1780249308300',
    data_pedido:           '15/07/2026',
    forma_pagamento:       'Boleto',
    condicao_pagamento:    '30/60/90',
    valor_liquido:         'R$ 4.759,42',
    tem_planilha_cadastro: true,
  },
}
